...
### Test Scenario M: Executive Analytics & Profitability Reports (Screen 10 — `/reports`)
Navigate to `/reports` via the sidebar navigation link **"Executive Reports"**.

#### Step 1: Verify the Morning Survival Snapshot
- Observe the 4 KPI cards at the top of the dashboard:
  - **Total Liquid Cash**: `PKR 12,092,655` (Combined Meezan Bank and physical Office Safe balance).
  - **Client Funds Held**: Amber warning card displaying unallocated client advances & seller escrow funds.
  - **Total Receivables (AR)**: `PKR 32,000,000` (Unpaid client installment milestones).
  - **Total Payables (AP)**: `PKR 1,542,500` (Pending vendor/supplier bills queued for Thursday payment run).

#### Step 2: Verify Corporate Profitability & True Net Income
- Inspect the mathematical waterfall card:
  - **Gross Deal Profit**: `PKR 5,000,000` (Realized profit across construction and sales deals).
  - **Brokerage Cut**: `+PKR 200,000` (Earned commission cuts).
  - **Less: Office Overhead**: `-PKR 694,845` (Screen 5 bills without project tag: rent, utilities, staff salaries, tea).
  - **True Net Income**: Displayed in deep emerald gradient: **`+PKR 4,505,155`** (Actual wealth generated by Wadaan for the fiscal period).

#### Step 3: Verify Deal-by-Deal Profit Ledger
- Inspect the deal table:
  - Look at **`Chaudhry Aslam`** (`CONSTRUCTION` on `Wadaan Heights`):
    - Contract Value: `PKR 16,000,000`
    - Collected Revenue: `PKR 8,000,000`
    - WIP Project Cost: `PKR 3,000,000`
    - Gross Profit: `PKR 5,000,000`
    - Margin %: `62.5%` (emerald green badge).
- Click the row:
  - The **`DealMarginDetailDrawer`** slides out from the right displaying the financial breakdown and unit economics.
  - Click **"Close Drill-Down"** or `Escape` to close.

#### Step 4: Verify the Aging Radar (Who is Late? Tracker)
- **Receivables Aging (Money In)**:
  - `Tariq Mehmood`: Down Payment `PKR 3,000,000` (`3 Days Late` red badge).
  - `Tariq Mehmood`: Full Contract Lump Sum `PKR 10,000,000` (`3 Days Late` red badge).
- **Payables Aging (Money Out)**:
  - `Car Wash`: `PKR 500` (`5 Days Waiting`).
  - `Petrol Pump`: `PKR 42,000` (`3 Days Waiting`).
  - `Ali Hardware`: `PKR 1,500,000` (`3 Days Waiting`).

#### Step 5: Verify 1-Click Print & PDF Generation
- Click the top right **"Generate PDF / Print"** button.
- *Verification*: The print layout activates, concealing the sidebar navigation and action buttons (`no-print`), and displaying a clean, branded **"WADAAN REAL ESTATE & BUILDERS — Executive Financial Intelligence & Profitability Statement"** document.

---

## ⚡ 5. Terminal Automated Test Commands

To re-verify all 194 tests across the entire application:

```powershell
# Run backend test suite (52 tests in 9 suites)
npm test --prefix backend

# Run frontend test suite (142 tests in 22 suites)
